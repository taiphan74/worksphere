  ...
  return (
    ...
    >
      ...
      >
        ...
        <CardContent className="flex flex-col gap-8 overflow-y-auto p-5 no-scrollbar sm:p-6">
          <div className="space-y-4">
            <Input
              value={title}
              onChange={(event) => setTitle(event.target.value)}
              placeholder={t("taskNamePlaceholder")}
              className="h-14 rounded-none border-0 bg-transparent px-0 text-2xl font-bold text-neutral-900 shadow-none placeholder:text-neutral-400 focus-visible:ring-0 md:text-3xl"
            />
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <MetaChip
              icon={CircleEllipsis}
              label={t("more")}
              className="cursor-default bg-white/15"
            />
          </div>

          <div className="space-y-4 rounded-[24px] border border-dashed border-white/40 bg-white/5 p-5 transition-all hover:bg-white/10 sm:p-6">
        ...
        <CardFooter className="flex flex-col gap-4 border-t border-white/20 px-5 py-4 sm:px-6 sm:py-5 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button type="button" size="sm" className="rounded-full px-5" onClick={handleCreateTask}>
              {t("create")}
            </Button>
          </div>
        </CardFooter>
